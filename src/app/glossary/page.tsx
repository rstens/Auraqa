...
function GlossaryContent() {
  ...
  const filteredTerms = useMemo(() => {
    ...
    if (activeCategories.size > 0) {
      terms = terms.filter((t) => activeCategories.has(t.category));
    }
    if (acronymsOnly) {
      terms = terms.filter((t) => t.abbreviation !== null);
    }
    ...
    return terms;
  }, [deferredQuery, activeCategories, acronymsOnly]);
  ...
  function renderDefinitionWithLinks(definition: string, currentTermId: string) {
    ...
    while ((match = regex.exec(definition)) !== null) {
      ...
      if (!targetId || targetId === currentTermId) {
        continue;
      }
                      ...
                      <div data-testid={`glossary-related-${term.id}`}>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {term.relatedTerms.map((relId) => {
                            const rel = GLOSSARY_TERMS.find((t) => t.id === relId);
                            if (!rel) return null;
                      ...
                      <div data-testid={`glossary-seealso-${term.id}`}>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {term.seeAlso.map((seeId) => {
                            const see = GLOSSARY_TERMS.find((t) => t.id === seeId);
                            if (!see) return null;
                            return (
                              <button
                                key={seeId}
